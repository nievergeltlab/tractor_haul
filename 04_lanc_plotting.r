...
if (length(args) != 2) {
  print("Usage: Rscript lanc_plotting.r {ancestry directory} {study name}")
}
#reference data file name
dir <- args[1]
study <- args[2]
outdir <- paste(dir, 'plots', sep='/')
...
for (i in c(1:22))
{
  preddir <- paste(dir, "predictions", study, i, sep="/")
	assign(paste("chr_", i, sep=""), fread(paste(preddir, "/", study, "_", i, ".msp.tsv", sep=""), data.table=F))
}
